// METAMASK CONNECTION
const TIMEOUT = 1000;
const COLLECTION_NAME = 'MOON PASS';
let editions = [];
let dots = 1;
...
function updateStatusText(isOwner, checking) {
  ...
  if(checking) {
    if(isOwner) {
      ...
      statusText.innerText = `Checking to see if you own a ${COLLECTION_NAME} ${renderDots(dots)}`;
    }
  } else {
    if(isOwner) {
      statusText.innerText = `You own ${editions.length} ${COLLECTION_NAME}!! `;
    } else {
      statusText.innerText = `You don't own a ${COLLECTION_NAME} `;
    }
  }
